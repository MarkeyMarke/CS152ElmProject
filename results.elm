...
view actionAddress model =
    let
        ...
        array =
            Array.fromList codeList

        tuple =
            Maybe.withDefault ( "", { choice1 = "", choice2 = "", choice3 = "", choice4 = "", c1Votes = [], c2Votes = [], c3Votes = [], c4Votes = [] } ) (Array.get 0 array)
        ...
        c1Votes =
            convertStr (List.length item.c1Votes)

        c2Votes =
            convertStr (List.length item.c2Votes)

        c3Votes =
            convertStr (List.length item.c3Votes)

        c4Votes =
            convertStr (List.length item.c4Votes)
    in
        div []
            [ fieldset []
                [ div []
