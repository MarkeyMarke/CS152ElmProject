...
updateState action model =
    case action of
        ...
        ClearVotes ->
            ( model, effectItemsString model.apiKey <| ElmFire.Op.remove "Votes" )
...
view actionAddress model =
    let
        ...
        singleList =
            List.filter codeChecker itemList

        array =
            Array.fromList singleList

        tuple =
            Maybe.withDefault ( "", { question = "", choice1 = "", choice2 = "", choice3 = "", choice4 = "" } ) (Array.get 0 array)
...
questionMaker address textValue index =
    div []
        [ --beginning of radio button
          label
            [ style [ ( "padding", "20px" ) ] ]
            [ input [ type' "radio", name "question", on "input" targetChecked (\bool -> Signal.message address (SetTempAnswer bool index)) ] [], text ((indexToLetter index) ++ ")") ]
